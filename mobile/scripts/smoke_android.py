...
try:
    ...
    launch()
    wait_label("Что вы ищете?")
    wait_label("Выбрать город")
    wait_label("Найти")
    wait_label("Оценить")
    wait_label("Профиль")
    tap("Меню приложения")
    wait_label("Рейтинги")
    menu_xml = (OUTPUT / "last-ui.xml").read_text()
    assert "Вход администратора" not in menu_xml and "Кабинет бизнеса" not in menu_xml
    tap("Закрыть")
    shot("01-search")
    tap("Оценить")

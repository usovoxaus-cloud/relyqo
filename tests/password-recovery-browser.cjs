...
(async()=>{
  ...
  try{for(const width of [1440,390,320]){
    ...
    await page.goto(base+'/reset-password');assert.equal(await page.locator('#resetForm').isVisible(),false);await capture('reset-missing');
    await page.goto(base+'/reset-password#token='+'x'.repeat(43));await page.waitForFunction(()=>!location.hash);
    ...
    await page.route('**/v1/auth/reset-password',r=>r.fulfill({status:400,json:{detail:'Ссылка недействительна или срок её действия истёк. Запросите новое письмо.'}}));

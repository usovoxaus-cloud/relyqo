...
test('country directory starts without GPS or Maps, lists unrated places, and limits automatic AI to the selected city',async()=>{
 ...
 document.querySelector('#ratedCountry').value='UZ';document.querySelector('#ratedCountry').dispatchEvent(new events.Event('change'));await settle();document.querySelector('#ratedCity').value='Tashkent';document.querySelector('#ratedCity').dispatchEvent(new events.Event('change'));await settle();const auto=[...timers.values()][0];timers.clear();auto();await settle();const ai=requests.find(r=>r.url==='/v1/public/advisor');assert.deepEqual(JSON.parse(ai.options.body).candidates.map(r=>r.object_key),['manual:one']);assert.match(document.querySelector('#results').textContent,/School/);
 ...
 assert.match(document.querySelector('#ratedCity').textContent,/Самарканд/);document.querySelector('#ratedCountry').value='KZ';document.querySelector('#ratedCountry').dispatchEvent(new events.Event('change'));await settle();assert.equal(document.querySelector('#ratedCity').value,'ALL');assert.match(document.querySelector('#ratedCity').textContent,/Алматы/);assert.doesNotMatch(document.querySelector('#ratedCity').textContent,/Самарканд/);document.querySelector('#ratedCity').value='Almaty';document.querySelector('#ratedCity').dispatchEvent(new events.Event('change'));await settle();assert.match(requests.at(-1).url,/country_code=KZ/);assert.match(requests.at(-1).url,/city=Almaty/);assert.equal(document.querySelector('#ratedCountry').value,'KZ');assert.equal(document.querySelector('#ratedCity').value,'Almaty');assert.equal(document.querySelectorAll('#results .place').length,0);assert.match(document.querySelector('#results').textContent,/Каталог пополняется/);assert.equal(document.querySelector('#results a').getAttribute('href'),'/business-owner');
});

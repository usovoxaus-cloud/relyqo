...
test('Uzbekistan directory starts without GPS, lists unrated places, and scopes the ratings advisor to a city',async()=>{
 ...
 const rows=[{kind:'manual',id:'one',object_key:'manual:one',name:'School',city:'Tashkent',country_code:'UZ',region_code:'13',category:'EDUCATION',address:'Street 1',community_rating_count:2,community_score:80,verified_rating_count:0},{kind:'manual',id:'two',object_key:'manual:two',name:'New clinic',city:'Samarkand',country_code:'UZ',region_code:'10',category:'HEALTH',address:'Street 2',community_rating_count:0,community_score:0,verified_rating_count:0}];
 const geo=[{country_code:'UZ',card_count:2,cities:[{city:'Tashkent',card_count:1},{city:'Samarkand',label_ru:'Самарканд',label_uz:'Samarqand',card_count:1}]},{country_code:'KZ',card_count:0,cities:[{city:'Almaty',label_ru:'Алматы',label_uz:'Olmaota',card_count:0}]}];
 const window={relyqoUzbekistan:JSON.parse(read('uzbekistan.json')),relyqoCategoriesReady:Promise.resolve([]),setTimeout:fn=>{timers.set(++clock,fn);return clock},clearTimeout:id=>timers.delete(id)};
 const context={document,window,Option:function(text,value=text){const option=document.createElement("option");option.textContent=text;option.value=value;return option;},URLSearchParams,Intl,AbortController,setTimeout:window.setTimeout,clearTimeout:window.clearTimeout,localStorage:{getItem:()=>null,setItem(){}},navigator:{language:'ru',geolocation:{getCurrentPosition(){gps++}}},fetch:async(url,options)=>{
  ...
  assert(url.startsWith('/v1/public/rated-organizations?'));const p=new URLSearchParams(url.split('?')[1]),items=rows.filter(row=>(!p.get('city')||row.city===p.get('city'))&&(!p.get('country_code')||row.country_code===p.get('country_code')));return reply({items,total:items.length,has_more:false,geography:geo,facets:{countries:['UZ','KZ'],cities:geo.flatMap(country=>country.cities.map(row=>({...row,country_code:country.country_code})))}});
 }};
 vm.runInNewContext(read('nearby.js'),context);await settle();assert.equal(document.querySelector('#error').textContent,'');assert.equal(gps,0);assert.equal(requests.length,1);assert.match(document.querySelector('#results').textContent,/New clinic/);assert.equal(document.querySelectorAll('#directoryGeography button').length,0);assert.equal(document.body.classList.contains('directoryMode'),true);
 document.querySelector('#ratedRegion').value='13';document.querySelector('#ratedRegion').dispatchEvent(new events.Event('change'));await settle();document.querySelector('#ratedCity').value='Tashkent';document.querySelector('#ratedCity').dispatchEvent(new events.Event('change'));await settle();const auto=[...timers.values()][0];timers.clear();auto();await settle();const ai=requests.find(r=>r.url==='/v1/public/advisor');assert.deepEqual(JSON.parse(ai.options.body).candidates.map(r=>r.object_key),['manual:one']);assert.match(document.querySelector('#results').textContent,/School/);
 document.querySelector('#ratedRegion').value='10';document.querySelector('#ratedRegion').dispatchEvent(new events.Event('change'));await settle();document.querySelector('#ratedCity').value='Samarkand';document.querySelector('#ratedCity').dispatchEvent(new events.Event('change'));await settle();assert.match(requests.at(-1).url,/city=Samarkand/);assert.match(document.querySelector('#results').textContent,/New clinic/);assert.doesNotMatch(document.querySelector('#results').textContent,/School/);
 assert.equal(document.querySelector('#ratedCountry').value,'UZ');assert.match(requests.at(-1).url,/region_code=10/);assert.match(document.querySelector('#ratedCity').textContent,/Самарканд/);assert.doesNotMatch(document.querySelector('#ratedCity').textContent,/Алматы/);
});

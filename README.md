...
Установите зависимости `python -m pip install ".[dev]"`. Для проверки с отдельной
базой задайте `DATABASE_URL=sqlite:///./relyqo-local.db`, затем выполните
`python -m alembic upgrade head` и `python -m uvicorn app.main:app --port 8765`.
Открывайте `http://127.0.0.1:8765/nearby`. Файлы `.env` автоматически не читаются
классом Settings: переменные нужно передать процессу либо использовать
`uvicorn --env-file .env app.main:app --port 8765` (при миграции передайте те же переменные).

Карта читает только `GOOGLE_MAPS_BROWSER_KEY`; переменные `NEXT_PUBLIC_*` и
`VITE_*` этот Python-проект не использует. В Render ключ задаётся в Environment
сервиса `relyqo` (`sync: false` в `render.yaml` не содержит самого значения).
Для локальной карты существующий ключ должен разрешать адрес localhost,
Maps JavaScript API и Places API (New). Без ключа собственный каталог работает,
но улицы Google Maps и внешний поиск недоступны. При сетевом сбое карта завершает
ожидание через 12 секунд; кнопка поиска позволяет повторить загрузку.

Браузерные регрессионные проверки: при установленном Node.js и пакете Playwright
выполните `node tests/map-browser.cjs` с работающим локальным сервером.
По умолчанию используется Edge и порт 8765; адрес задаётся `RELYQO_TEST_URL`,
канал браузера — `BROWSER_CHANNEL`. Google API подменяется только в тесте:
проверяются отсутствие ключа, повторная загрузка, таймаут, сетевой сбой,
ранний/поздний отказ авторизации и отказ геолокации без расходования квоты Google.

...
## Восстановление пароля потребителя

Используется существующая авторизация RELYQO (FastAPI, `users`, scrypt, серверные cookie-сессии), не Supabase. В кабинете `/me` потребитель указывает email и текущий пароль, затем подтверждает адрес по письму. Только подтверждённый адрес используется на `/forgot-password`. Регистрация по имени пользователя сохранена; новым и существующим пользователям следует подтвердить email в кабинете. Аккаунт, пароль от которого уже утрачен и почта к которому никогда не привязывалась, нельзя безопасно восстановить автоматически.
...
Перед запуском новой версии примените `alembic upgrade head` (миграция `0020`). Существующий Docker CMD уже выполняет миграции перед стартом. Миграция добавляет `consumer_emails`, `password_recovery_tokens`, `recovery_rate_limits`, не меняя существующие учётные записи. Перед production-миграцией нужна резервная копия БД.

Публичные страницы: `/forgot-password`, `/reset-password`, `/verify-email`. Ссылка из письма содержит токен во fragment `#token=…`, который не отправляется веб-серверу и удаляется из адресной строки после чтения. Токен передаётся backend только в JSON POST. Redirect после сброса не принимается от пользователя: предусмотрен обычный переход на `/me`, без автоматического входа. Эти страницы не подключают рекламу, аналитику или сторонние скрипты; выставлены CSP, `no-store`, `no-referrer`, запрет iframe.

### Гарантии и ограничения

- 32 случайных байта, хранится SHA-256 хеш; срок 30 минут; атомарное однократное использование, включая параллельные запросы.
- Токен привязан к роли CONSUMER, аккаунту, адресу и текущему хешу пароля. Смена пароля другим способом делает старую ссылку недействительной.
...
- DB-backed лимиты: 30 запросов/час на ASGI client IP, 3 письма/час на email, 3 попытки привязки/час на пользователя. Доверять forwarded IP можно только от фактического reverse proxy; иначе общий IP прокси может ограничить нескольких пользователей.
- При отказе почтового API токен инвалидируется; в логах только тип события, без адреса, ключа или ссылки. Ответ «запрос принят» не является подтверждением доставки. BackgroundTasks не является устойчивой очередью: при остановке процесса письмо может не отправиться, пользователь должен повторить запрос. Перед production обязательно проверить реальное письмо, срок ссылки, вход после сброса и уведомление о смене пароля.

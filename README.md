...
Страница `/nearby` — собственная RELYQO Map. С разрешения пользователя она определяет текущее местоположение и показывает организации из каталога RELYQO в выбранном вручную радиусе от 1 до 50 км. Пользователь отдельно выбирает сферу: питание, гостиницы, красота, здоровье, развлечения, магазины, автоуслуги или профессиональные услуги. Координаты используются только браузером для текущего запроса и не сохраняются RELYQO.

Карта не загружает внешние карты, каталоги или рейтинги и не требует внешнего API-ключа. Это начальная координатная карта RELYQO: точки размещаются относительно местоположения пользователя без уличной подложки. Зарегистрированные организации и места потребителей хранятся в собственной базе RELYQO. Результаты можно ограничить категорией, радиусом, количеством и избранным.

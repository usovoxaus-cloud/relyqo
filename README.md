...
Для проверки заявок задайте отдельный секрет `ADMIN_PASSWORD` и откройте `/admin`. Первый вход с именем `relyqo-admin` создаёт административный аккаунт. Решение `PUBLISH` переводит профиль в статус `PUBLISHED` и разрешает показывать его в каталоге, но не присваивает статус QR-партнёра, не создаёт Verified Score и не добавляет организацию в официальный рейтинг. Решение `REJECT` скрывает профиль. Если владелец изменит уже опубликованные сведения, профиль снова получает `SELF_REGISTERED` и должен пройти повторную проверку.

...
Страница `/nearby` с разрешения пользователя определяет его текущее местоположение и показывает организации в выбранном вручную радиусе от 1 до 50 км. Пользователь отдельно выбирает сферу: питание, гостиницы, красота, здоровье, развлечения, магазины, автоуслуги или профессиональные услуги. Список сортируется по расстоянию, рейтингу, числу оценок или названию. Координаты используются только браузером для текущего запроса и не сохраняются RELYQO. Зелёным отмечены подключённые партнёры RELYQO, синим — справочные результаты Google Maps. Пользователь может сохранить объект в избранное.
...
Вошедший потребитель может нажать **«Оценить в RELYQO»** у партнёра или объекта Google Maps и оставить отдельную общественную оценку качества. В базе хранится только идентификатор объекта (`place_id` для Google), сама оценка, идентификатор аккаунта потребителя и технический хеш защиты от повторной отправки; название, адрес и точная геолокация пользователя не сохраняются в Community-оценке. Один аккаунт может оценить один объект один раз.
...
Названия критериев оценки адаптируются к сфере, но внутренние числовые поля и формула остаются стабильными. Например, автосервис оценивается по качеству работы, срокам и сервису, аккуратности и прозрачности цены; гостиница — по комфорту, обслуживанию, чистоте и ценности. На карте и в профиле показывается детализация доступных показателей. Если внешний источник передаёт только общий рейтинг, RELYQO прямо сообщает, что детализации нет.

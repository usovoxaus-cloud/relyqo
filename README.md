...
Страница `/nearby` — RELYQO Map с визуальной подложкой Google Maps. С разрешения пользователя она определяет текущее местоположение и показывает организации из каталога RELYQO в выбранном вручную радиусе от 1 до 50 км. Пользователь отдельно выбирает сферу: питание, гостиницы, красота, здоровье, развлечения, магазины, автоуслуги или профессиональные услуги. Координаты используются только браузером для текущего запроса и не сохраняются RELYQO.

Google используется только для изображения улиц и навигации по карте. RELYQO не загружает Google Places, список организаций или Google Rating. Зарегистрированные организации и места потребителей хранятся в собственной базе RELYQO. Результаты можно искать по названию, адресу или городу, ограничить категорией, радиусом, количеством и избранным. Карточка позволяет открыть координаты объекта на Google Карте. Для подложки нужен ограниченный по домену ключ `GOOGLE_MAPS_BROWSER_KEY` с доступом только к Maps JavaScript API.
...
Импорт новых объектов из внешних каталогов отключён. Существующие исторические ссылки не удаляются из базы автоматически, чтобы не нарушить историю пользовательских действий, но они не показываются в активной карте и не пополняются.

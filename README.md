...
Страница `/nearby` с разрешения пользователя определяет его текущее местоположение и показывает рестораны в радиусе 15 км. Координаты используются только браузером для текущего запроса и не сохраняются RELYQO. Зелёным отмечены подключённые партнёры RELYQO, синим — справочные результаты Google Maps.

Данные и рейтинг Google показываются отдельно и никогда не входят в RELYQO Score. Оставить RELYQO-оценку можно только партнёру после Verified Visit по одноразовому QR. Nearby Search показывает до 20 наиболее релевантных результатов за один запрос и не является полным реестром всех ресторанов.
